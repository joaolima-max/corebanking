...
export default function Equipe() {
  const me = useMe(); const orgId = me.data?.orgId
  ...
  return (
    <div>
      <PageHeader title="Equipe" subtitle="Usuários internos da Bass · papéis atribuídos via RBAC" action={<Soon label="Convidar usuário" note="Convite de usuário interno — será ligado a /users nesta fase." />} />
      <Card><SectionTitle badge={<Chip tone="acc">/users</Chip>}>{rows.length} usuários</SectionTitle>
        <DataTable columns={cols} rows={rows} loading={users.isLoading || me.isLoading} error={users.error} emptyText="Nenhum usuário." /></Card>
    </div>
  )
}
