...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (res.status === 403) throw new ApiError(403, 'Acesso negado.')
  if (!res.ok) {
    const body = (await res.json().catch(() => ({}))) as Record<string, unknown>
    const message = typeof body?.message === 'string' ? body.message : `Erro ${res.status}`
    throw new ApiError(res.status, message, typeof body?.error === 'string' ? body.error : undefined)
  }
...
export const api = {
  ...
  post: <T>(path: string, data: unknown, token?: string) =>
    request<T>(path, { method: 'POST', body: JSON.stringify(data), headers: token ? { Authorization: `Bearer ${token}` } : {} }),
}
